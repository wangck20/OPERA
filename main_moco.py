...
def main_worker(gpu, ngpus_per_node, args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch % args.print_freq == 0:
            validate_during_training(val_loader, model, args)
        # validate_during_training(val_loader, model, args)
...
def train(train_loader, model, criterion, optimizer, scaler, epoch, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        with torch.cuda.amp.autocast(True):
            loss_con, output1, output2 = model(images[0], images[1], moco_m)

        loss = 0.5*loss_con + 0.5*criterion(output1, target) + 0.5*criterion(output2, target)
...
def validate_during_training(val_loader, model, args):
    ...
    with torch.no_grad():
        for i, (images, target) in enumerate(val_loader):
            ...
            output, cls0 = model(images, images, 0, 'val')
            acc1, acc5 = accuracy(cls0, target, topk=(1, 5))
            top1.update(acc1[0].item(), images.size(0))
            top5.update(acc5[0].item(), images.size(0))
            embeddings.append(output)
            targets.append(target)
            
        embeddings_cat = l2_norm(torch.cat(embeddings)).cpu()
        targets_cat = torch.cat(targets).cpu()
        torch.cuda.empty_cache()
        cos_sim = torch.nn.functional.linear(embeddings_cat, embeddings_cat)
        pre = targets_cat[cos_sim.topk(2)[1][:, 1:]]
        model.train()

        num = 0

        for i in range(targets_cat.size()[0]):
            if targets_cat[i] == pre[i]:
                num = num + 1

        r1 = float(num)/float(targets_cat.size()[0])
